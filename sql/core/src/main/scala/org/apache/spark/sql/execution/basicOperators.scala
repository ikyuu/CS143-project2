...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    //weijia task4
    //new iterator
    var newIterator: Iterator [Row] = null
    ...
    new Iterator[Row] {
      ...
      /**
        ...
        * @return
        */
        //define from the cs143.cachingiteratorgenerator return
        var cache: (Iterator[Row] => Iterator[Row]) = null
        // var cache: Iterator[Row] = null
        var diskIterator = DiskHashedRelation(input, keyGenerator).getIterator()
      ...
      private def fetchNextPartition(): Boolean  = {
        ...
        else
        {
          ...
          cache = CS143Utils.generateCachingIterator(projectList, child.output)
          diskPartition = diskIterator.next()
          newIterator = cache(diskPartition.getData())
          if (!newIterator.hasNext)
          {
            fetchNextPartition()
          }
