...
case class SpillableAggregate(
  ...
  def generateIterator(input: Iterator[Row], memorySize: Long = 64 * 1024 * 1024, numPartitions: Int = 64): Iterator[Row] = {
    ...
    def initSpills(): Array[DiskPartition] = {
      ...
      val dArray: Array[DiskPartition] = new Array[DiskPartition](numPartitions)
      for (i <- 0 until numPartitions)
      {
        dArray(i) = new DiskPartition(Integer.toString(i), 0)
      }
      dArray
    }
    ...
    new Iterator[Row] {
      ...
      private def aggregate(): Iterator[Row] = {
        ...
        while (data.hasNext) {
          ...
          var aggTable = currentAggregationTable(group)
          ...
          if (aggTable == null)
          {
            ...
            {
              aggTable = newAggregatorInstance()
              currentAggregationTable.update(group, aggTable)
            }
        ...
        val seqAgg = Seq(aggregatorSchema) ++ namedGroups.map(x => (x._2))
        AggregateIteratorGenerator(resultExpression, seqAgg)(currentAggregationTable.iterator)
      ...
      /**
        ...
        */
      // var hashedSpills: Option[Iterator[DiskPartition]] = None
      val dpIterator = spills.iterator

      private def fetchSpill(): Boolean  = {
        /* IMPLEMENT THIS METHOD */
        var foundit = dpIterator.hasNext
        
        // while (!data.hasNext)
        // {
        //   if (dpIterator.hasNext){
        //     data = dpIterator.next.getData()
        //   }
        //   else
        //   {
        //     break
        //   }
        // }
        while (dpIterator.hasNext && !data.hasNext)
        {
            data = dpIterator.next.getData()
        }
