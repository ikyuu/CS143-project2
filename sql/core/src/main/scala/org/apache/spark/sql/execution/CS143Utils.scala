...
object CS143Utils {
  ...
  def getUdfFromExpressions(expressions: Seq[Expression]): ScalaUdf = {
    ...
    var sUdf: ScalaUdf = null
    var i: Expression = null
    for ( i <- expressions )
    {
      if(i.isInstanceOf[ScalaUdf])
      {
        sUdf = i.asInstanceOf[ScalaUdf]
      }
    }
    sUdf
  }
    ...
    new Iterator[Row] {
      ...
      def next() = {
        /* IMPLEMENT THIS METHOD */
            ...
            val r : Row = input.next
            val pre : Row = preUdfProjection.apply(r)
            val post : Row = postUdfProjection.apply(r)
            val key : Row = cacheKeyProjection.apply(r)
            var data : Row = cache.get(key)
            if (data == null)
              {
                data = udfProject.apply(r)
                cache.put(key, data)
              }
            Row.fromSeq(pre ++ data ++ post)
      }
    }
  }
}
  ...
  def apply(resultExpressions: Seq[Expression],
    ...
    new Iterator[Row] {
      ...
      def next() = {
        ...
        val (row, func) = input.next()
         // val a = func.eval()
        val tmp = new GenericMutableRow(1)
        tmp(0) = func.eval()
        postAggregateProjection(new JoinedRow4(tmp, row))
      }
    }
  }
}
